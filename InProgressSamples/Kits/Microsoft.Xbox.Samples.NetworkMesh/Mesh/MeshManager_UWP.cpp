...
using namespace Windows::Networking::XboxLive;
...
{
    // load endpoint pair template for Secure Device Association.
    m_associationTemplate = XboxLiveEndpointPairTemplate::GetTemplateByName(secureDeviceAssociationTemplateName);
...
void MeshManager::Initialize(uint8 localConsoleId)
{
    ...
    if (m_associationTemplate != nullptr)
    {
        ...
        TypedEventHandler<Windows::Networking::XboxLive::XboxLiveEndpointPairTemplate^, Windows::Networking::XboxLive::XboxLiveInboundEndpointPairCreatedEventArgs^>^ associationIncomingEvent =
            ref new TypedEventHandler<Windows::Networking::XboxLive::XboxLiveEndpointPairTemplate^, Windows::Networking::XboxLive::XboxLiveInboundEndpointPairCreatedEventArgs^>(
                [this](Windows::Networking::XboxLive::XboxLiveEndpointPairTemplate^ associationTemplate, Windows::Networking::XboxLive::XboxLiveInboundEndpointPairCreatedEventArgs^ args)
        {
            OnAssociationIncoming(associationTemplate, args);
        });

        m_associationIncomingToken = m_associationTemplate->InboundEndpointPairCreated += associationIncomingEvent;
    }
    else
    {
    ...
    unsigned short sin6_port = htons(m_associationTemplate->AcceptorBoundPortRangeLower);
...
XboxLiveEndpointPairTemplate^ MeshManager::GetSecureDeviceAssociationTemplate()
{
    ...
    )
{
    ...
    if(AreSecureDeviceAddressesEqual(XboxLiveDeviceAddress::GetLocal(), secureDeviceAddress))
    {
...
{
    XboxLiveEndpointPair^ association = args->EndpointPair;
    if(association != nullptr)
    {
        ...
        MeshConnection^ newMeshConnected = AddConnection(association->RemoteDeviceAddress, association, true, ConnectionStatus::Connected);
        if(newMeshConnected != nullptr)
        {
            RefreshConnections();
        }
...
void MeshManager::OnAutoConnectWorkerThreadDoWork( Microsoft::Xbox::Samples::NetworkMesh::ProcessThreadsEventArgs^ args )
{
    ...
    XboxLiveEndpointPairTemplate^ secureDeviceAssociationTemplate = GetSecureDeviceAssociationTemplate();
    ...
    for each (MeshConnection^ meshConnection in allDisconnectedConnections)
    {
        ...
        LogComment( Utils::GetThreadDescription(L"THREAD: Calling CreateEndpointPairAsync") );

        meshConnection->SetConnectionInProgress(true);
        auto asyncOp = secureDeviceAssociationTemplate->CreateEndpointPairAsync(
            meshConnection->GetSecureDeviceAddress()
            );

        create_task( asyncOp )
        .then([this, meshConnection](task<XboxLiveEndpointPairCreationResult^> t)
        {
            LogComment(Utils::GetThreadDescription(L"THREAD: CreateEndpointPairAsync result"));
            LogComment("Connecting from address: " + meshConnection->GetSecureDeviceAddress()->GetSnapshotAsBase64());

            try
            {
                XboxLiveEndpointPairCreationResult^ associationResult = t.get();
                if (associationResult->Status != XboxLiveEndpointPairCreationStatus::Succeeded)
                {
                    LogCommentFormat(L"Connecting to remote machine failed: %s", meshConnection->GetConsoleName()->Data());
                }
                else
                {
                    XboxLiveEndpointPair^ association = associationResult->EndpointPair;
                    ...
                    MeshConnection^ newMeshConnection = GetConnectionFromSecureDeviceAddress(association->RemoteDeviceAddress);
...
void MeshManager::OnHeartbeatWorkerThreadDoWork( Microsoft::Xbox::Samples::NetworkMesh::ProcessThreadsEventArgs^ args )
{
    ...
    for each (MeshConnection^ meshConnection in allconnected)
    {
        ...
        XboxLiveEndpointPair^ association = meshConnection->GetAssociation();
        if(association != nullptr)
        {
...
void MeshManager::RefreshConnections()
{
    ...
    // Now, loop through template and find each connection and mark the ones that we found
    // And take note of anything in the template that is not yet in our internal list of connections.
    Windows::Foundation::Collections::IVectorView<XboxLiveEndpointPair^>^ associations = GetSecureDeviceAssociationTemplate()->EndpointPairs;
    for each (XboxLiveEndpointPair^ associationInTemplate in associations)
    {
        MeshConnection^ meshConnection = GetConnectionFromSecureDeviceAddress(associationInTemplate->RemoteDeviceAddress);
...
void MeshManager::OnAssociationChange( Windows::Networking::XboxLive::XboxLiveEndpointPairStateChangedEventArgs^ args, Windows::Networking::XboxLive::XboxLiveEndpointPair^ association )
{
    // Update this mesh member with the latest information
    switch(args->NewState)
    {
    case Windows::Networking::XboxLive::XboxLiveEndpointPairState::DeletingLocally:
    case Windows::Networking::XboxLive::XboxLiveEndpointPairState::RemoteEndpointTerminating:
    case Windows::Networking::XboxLive::XboxLiveEndpointPairState::Invalid:
        {
            bool bFound = false;
            {
...
MeshConnection^ MeshManager::GetConnectionFromAssociation(Windows::Networking::XboxLive::XboxLiveEndpointPair^ association)
{
    if(association == nullptr)
    {
        return nullptr;
    ...
    XboxLiveDeviceAddress^ remoteSecureDeviceAddress = association->RemoteDeviceAddress;
    return GetConnectionFromSecureDeviceAddress(remoteSecureDeviceAddress);
}

MeshConnection^ MeshManager::GetConnectionFromSecureDeviceAddress(Windows::Networking::XboxLive::XboxLiveDeviceAddress^ address)
{
    if(address == nullptr)
    {
        return nullptr;
    ...
    for each (MeshConnection^ meshConnection in allConnections)
    {
        XboxLiveDeviceAddress^ remoteSecureDeviceAddress = meshConnection->GetSecureDeviceAddress();
...
void MeshManager::DestroyConnection( MeshConnection^ connection )
{
    ...
    XboxLiveEndpointPair^ meshAssociation = connection->GetAssociation();
    if(meshAssociation != nullptr)
    {
        if(false == connection->IsConnectionDestroying())
        {
            connection->SetConnectionDestroying(true);
            IAsyncAction^ asyncOp = meshAssociation->DeleteAsync();
            create_task(asyncOp)
                .then([this] (task<void> t)
            {
                ...
                {
                    LogCommentFormat( L"MeshManager::DestroyConnection - DeleteAsync failed %s", Utils::GetErrorString(ex->HResult)->Data());
                }
            });
        }
...
void MeshManager::DisconectFromAddress( Windows::Networking::XboxLive::XboxLiveDeviceAddress^ address )
{
...
void MeshManager::DestroyAndDisconnectAll()
{
    ...
    for each (MeshConnection^ meshConnection in allConnections)
    {
        ...
        {
            LogCommentFormat(L"MeshManager::DestroyAndDisconnectAll - DestroyConnection failed %s", Utils::GetErrorString(ex->HResult)->Data());
        }
    }
...
void MeshManager::DestroyAllTemplateAssociations()
{
    if(GetSecureDeviceAssociationTemplate() != nullptr)
    {
        ...
        for each (XboxLiveEndpointPair^ associationInTemplate in associations)
        {
            IAsyncAction^ asyncOp = associationInTemplate->DeleteAsync();
            create_task(asyncOp)
                .then([this] (task<void> t)
            {
                try
                ...
                {
                    LogCommentFormat( L"MeshManager::DestroyAllTemplateAssociations - DeleteAsync failed %s", Utils::GetErrorString(ex->HResult)->Data());
                }
            }).wait();
        }
    }
...
void MeshManager::Shutdown()
{
    ...
    if( m_associationTemplate != nullptr )
    {
        m_associationTemplate->InboundEndpointPairCreated -= m_associationIncomingToken;
...
bool MeshManager::AreSecureDeviceAddressesEqual( Windows::Networking::XboxLive::XboxLiveDeviceAddress^ secureDeviceAddress1, Windows::Networking::XboxLive::XboxLiveDeviceAddress^ secureDeviceAddress2 )
{
...
void MeshManager::OnHelloReceived( Microsoft::Xbox::Samples::NetworkMesh::MeshHelloReceivedEvent^ args )
{
    if(args != nullptr)
    {
        ...
        if(meshConnection != nullptr )
        {
            XboxLiveEndpointPair^ secureDeviceAssociation = meshConnection->GetAssociation();
