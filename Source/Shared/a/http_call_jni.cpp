...
static xbox::services::http_call* getHttpCall(JNIEnv * env, jobject httpCall)
{
    ...
    return reinterpret_cast<std::shared_ptr<xbox::services::http_call>*>(env->GetLongField(httpCall, id))->get();
}
...
static bool prepare_stream(JNIEnv * myEnv, std::shared_ptr<xbox::services::http_call_response> response, jobject& stream, jmethodID& closeStream)
{
    auto v = response->response_body_vector();
    auto sz = v.size();
    LOGD("Received %d bytes", sz);
...
static bool prepare_headers(JNIEnv* myEnv, std::shared_ptr<xbox::services::http_call_response> response, jclass clsHeaders, jobject& headers)
{
    ...
    (JNIEnv * env, jobject httpCall, jobject callback)
{
    ...
    xbox::services::http_call* http_call = getHttpCall(env, httpCall);
    auto task = http_call->get_response_with_auth(
        xbox::services::http_call_response_body_type::vector_body
        )
        .then([jvm, myCallback, myClsHeaders](std::shared_ptr<xbox::services::http_call_response> response)
    {
        JNIEnv* myEnv;
    ...
    (JNIEnv * env, jclass cls, jstring method, jstring endpoint, jstring pathAndQuery, jboolean addDefaultHeaders)
{
    ...
    std::shared_ptr<xbox::services::xbox_live_context_settings> ctx(new xbox::services::xbox_live_context_settings());

    std::shared_ptr<xbox::services::http_call>* http_call = new std::shared_ptr<xbox::services::http_call>(
...
{
    delete reinterpret_cast<std::shared_ptr<xbox::services::http_call>*>(id);
}
