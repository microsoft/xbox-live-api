...
#define INVOKE_CALLBACK_WITH_INVALIDARGUMENT_IF(x, callback, type, message) { if ( x ) { callback(xbox::services::xbox_live_result<type>(xbox_live_error_code::invalid_argument, message)); } }
...
#define INVOKE_CALLBACK_WITH_INVALIDARGUMENT_IF(x, callback, type, message) { assert (!(x)); if ( x ) { callback(xbox::services::xbox_live_result<type>(xbox_live_error_code::invalid_argument, message)); } }
