...
)
{
    ...
    auto xblUser = xbox_live_user::get_last_signed_in_user();
    ...
    if (showProfileCardUIMethodId != NULL && xblUser != nullptr)
    {
        title_callable_ui_internal::s_isTcuiRunning = true;
        auto userXuid = xblUser->xbox_user_id();
        auto userPrivileges = xblUser->privileges();
        auto currentUserIdString = jniEnv->NewStringUTF(userXuid.c_str());
        auto targetUserIdString = jniEnv->NewStringUTF(targetXboxUserId.c_str());
        auto currentUserPrivileges = jniEnv->NewStringUTF(userPrivileges.c_str());
        jniEnv->CallStaticVoidMethod(interopTcuiClass, showProfileCardUIMethodId, activity, currentUserIdString, targetUserIdString, currentUserPrivileges);
    }
    else
    {
...
title_callable_ui::show_friend_finder_ui()
{
    ...
    JNIEnv* jniEnv;
    JNI_ATTACH_THREAD(jvm, jniEnv);
    jmethodID showFriendFinderMethodId = jniEnv->GetStaticMethodID(interopTcuiClass, "ShowFriendFinder", "(Landroid/app/Activity;Ljava/lang/String;Ljava/lang/String;)V");
    if (showFriendFinderMethodId != NULL && xblUser != nullptr)
    {
        title_callable_ui_internal::s_isTcuiRunning = true;
        auto userXuid = xblUser->xbox_user_id();
        auto userPrivileges = xblUser->privileges();
        auto currentUserIdString = jniEnv->NewStringUTF(userXuid.c_str());
        auto currentUserPrivileges = jniEnv->NewStringUTF(userPrivileges.c_str());
        jniEnv->CallStaticVoidMethod(interopTcuiClass, showFriendFinderMethodId, activity, currentUserIdString, currentUserPrivileges);
    }
...
)
{
    ...
    JNIEnv* jniEnv;
    JNI_ATTACH_THREAD(jvm, jniEnv);
    jmethodID showUserProfileMethodId = jniEnv->GetStaticMethodID(interopTcuiClass, "ShowUserProfile", "(Landroid/content/Context;Ljava/lang/String;)V");
